...
const ViewMyRecords = () => {
  ...
  const handleContractError = (error, context) => {
    ...
    } else if (error.message?.includes("Not a registered patient")) {
  ...
  const fetchMyRecords = async () => {
    ...
    try {
      ...
      if (!isPatient) {
        setError("You need to register as a patient to view medical records");
        setRecords([]);
        return;
      }
      ...
      const formattedRecords = myRecords.map((record, index) => ({
        id: record.recordId?.toString() || index.toString(),
...
const handleDownloadRecord = async (record) => {
  try {
    // Call backend to get decrypted file
    ...
    const fileName = `medical-record-${record.id}.${fileExtension}`;
    ...
    console.log(`Downloading decrypted record from backend: ${record.ipfsHash}`);
      ...
      {records.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">
            {walletAddress ? "You haven't uploaded any medical records yet." : "Please connect your wallet to view medical records."}
          </p>
          {walletAddress && (
            <button
              onClick={handleGoToUpload}
